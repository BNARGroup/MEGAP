...
function [cleanData, zaplineConfig, analyticsResults, plothandles] = clean_data_with_zapline_plus(data, srate,MEGAP_variables, varargin)
...
disp('This script has been modified by MEGAP. For the original script, please refer to the Zapline_plus repository.')
disp(' ')
disp(' ')
disp(' ')
disp('---------------- PLEASE CITE ------------------')
...
disp('For MEGAP, Zapline_plus is configured with a "minfreq" of 10 Hz and a "maxfreq" set to the final harmonic of 50 or 60 Hz, just below the cHPI frequencies.')
sensivity=MEGAP_variables(1);
max_harmonic=MEGAP_variables(2);
...
addOptional(p, 'minfreq', 10, @(x) validateattributes(x,{'numeric'},{'positive','scalar'},'clean_EEG_with_zapline','minfreq'))
addOptional(p, 'maxfreq', max_harmonic, @(x) validateattributes(x,{'numeric'},{'positive','scalar'},'clean_EEG_with_zapline','maxfreq'))
addOptional(p, 'detectionWinsize', 6, @(x) validateattributes(x,{'numeric'},{'positive','scalar'},'clean_EEG_with_zapline','detectionWinsize'))
addOptional(p, 'coarseFreqDetectPowerDiff', sensivity, @(x) validateattributes(x,{'numeric'},{'positive','scalar'},'clean_EEG_with_zapline','coarseFreqDetectPowerDiff'))%defult 4
